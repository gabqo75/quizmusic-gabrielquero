...
$stmt = $pdo->query("
    SELECT q.*, COUNT(ques.id) AS nb_questions
    FROM questionnaires q
    LEFT JOIN questions ques ON q.id = ques.questionnaire_id
    WHERE q.actif = 1
    GROUP BY q.id
                ...
                <div class="bg-gradient-to-r <?php echo $quiz['couleur']; ?> p-6 text-white">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-sm font-medium bg-white/20 px-2 py-1 rounded-full">
                            <?php echo htmlspecialchars($quiz['nb_questions']); ?> question<?php echo $quiz['nb_questions'] > 1 ? 's' : ''; ?>
                        </span>
